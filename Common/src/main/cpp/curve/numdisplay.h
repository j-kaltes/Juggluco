...
//	nvgEllipse(vg, x, y , foundPointRadius,foundPointRadius/4);
...
//	int nr=m[mealptr].ingr; if(nr>mealptr) return;
	...
	for(int i=start;i<mealptr;i++) {
		nvgText(vg, x,y,meals->datameal()->ingredients[m[i].ingr].name.data(),NULL );
		y+=smallfontlineheight;
		}
/*struct mealposition {
	float mealx;
...
template <int N>
const Num *findpast() const {
	return findpast(std::get<N>(extrum));
	}
template <int N=1>
const Num *findforward() const {
	const Num *en=end();
